...
public class SdkHttpTask extends AsyncTask<String, Void, String> {
	...
	public void doPost(SdkHttpListener listener, String keyValueArray,
			String url) {
	...
	protected String doInBackground(String... params) {
		...
		while (response == null && mRetryCount < MAX_RETRY_TIME) {
			...
			try {
				...
				//LogHelper.d(TAG, this.toString() + "||mRetryCount="+ mRetryCount);
				//LogHelper.d(TAG, this.toString() + "||request=" + uri);
				...
				if (httpResp != null && !isCancelled()) {
					...
					LogHelper.d(TAG, this.toString() + "||status =" + status);
			...
			//LogHelper.d(TAG, this.toString() + "||response=" + response);
	...
	protected void onCancelled() {
		...
		if (mListener != null) {
			//LogHelper.d(TAG, this.toString() + "||onCancelled");
	...
	protected void onPostExecute(String response) {
		...
		if (mListener != null && !isCancelled()) {
			//LogHelper.d(TAG, this.toString() + "||onResponse");
	...
	private HttpResponse executeHttp(Context context, String uri)
		...
		return mIsHttpPost ? post(context, uri, mKeyValueArray) : get(context,
				uri);
	}
